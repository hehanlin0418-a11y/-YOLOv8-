...
def draw_keypoints_and_status(frame, keypoints, status, fall_details=None):
    ...
    try:
        ...
        if fall_details and status == "Falling":
            ...
            cv2.putText(frame_copy, f"Angle: {angle:.1f}deg", (10, 60),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
            cv2.putText(frame_copy, f"Chest>Waist: {y_comp}", (10, 90),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
            cv2.putText(frame_copy, f"Aspect Ratio: {aspect:.2f}", (10, 120),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
