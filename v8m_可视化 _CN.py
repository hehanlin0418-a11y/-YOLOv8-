...
def configure_chinese_font():
    """
    自动配置中文概字体，防止乱码。
    在调用 plt.style.use 之后必须再次调用此函数。
    """
    system_name = platform.system()

    # 定义不同系统的常用中文字体列表 (优先级从高到低)
    if system_name == "Windows":
        ...
        font_list = ['WenQuanYi Micro Hei', 'Droid Sans Fallback', 'SimHei', 'DejaVu Sans']
    ...
    # 如果没找到标准名称，尝试直接设置rcParams，Matplotlib有时能自动回退
...
def plot_per_video_metrics(video_names, metrics):
    ...
    rects1 = ax.bar(x - width * 1.5, metrics['prec'], width, label='精确率 (Precision)', color=colors[0],
                    edgecolor='black', linewidth=1.5, alpha=0.9, zorder=3)
    rects2 = ax.bar(x - width * 0.5, metrics['rec'], width, label='召回率 (Recall)', color=colors[1],
                    edgecolor='black', linewidth=1.5, alpha=0.9, zorder=3)
    rects3 = ax.bar(x + width * 0.5, metrics['f1'], width, label='F1分数 (F1 Score)', color=colors[2],
                    edgecolor='black', linewidth=1.5, alpha=0.9, zorder=3)
    rects4 = ax.bar(x + width * 1.5, metrics['acc'], width, label='准确率 (Accuracy)', color=colors[3],
                    edgecolor='black', linewidth=1.5, alpha=0.9, zorder=3)

    ax.set_ylabel('分数', fontsize=14, fontweight='bold')
    ax.set_xlabel('视频', fontsize=14, fontweight='bold')
    ax.set_title('各视频性能指标', fontsize=16, fontweight='bold', pad=20)
    ax.set_xticks(x)
...
def plot_confusion_matrix(y_true, y_pred):
    ...
    ax.set_title('混淆矩阵', fontsize=16, fontweight='bold', pad=15)
...
def plot_pr_roc(y_true, y_pred, fall_scores):
    ...
    ax1.plot(recall, precision, 'b-', linewidth=3, label=f'PR曲线 (AUC = {auc_pr:.3f})', alpha=0.8)
    ax1.fill_between(recall, precision, alpha=0.2, color='blue')
    ax1.set_xlabel('召回率 (Recall)', fontsize=14, fontweight='bold')
    ax1.set_ylabel('精确率 (Precision)', fontsize=14, fontweight='bold')
    ax1.set_title('精确率-召回率曲线 (PR Curve)', fontsize=16, fontweight='bold', pad=15)
    ax1.legend(loc='best', fontsize=12, framealpha=0.9, shadow=True)
    ax1.grid(True, alpha=0.4)
    ax1.set_xlim([0, 1])
    ax1.set_ylim([0, 1])

    ax2.plot(fpr, tpr, 'r-', linewidth=3, label=f'ROC曲线 (AUC = {auc_roc:.3f})', alpha=0.8)
    ax2.plot([0, 1], [0, 1], 'k--', alpha=0.6, linewidth=2)
    ax2.fill_between(fpr, tpr, alpha=0.2, color='red')
    ax2.set_xlabel('假阳性率 (False Positive Rate)', fontsize=14, fontweight='bold')
    ax2.set_ylabel('真阳性率 (True Positive Rate)', fontsize=14, fontweight='bold')
    ax2.set_title('ROC 曲线', fontsize=16, fontweight='bold', pad=15)
...
def plot_performance_summary(all_metrics):
    ...
    metrics_names = ['精确率', '召回率', 'F1分数', '准确率', '特异度', '平衡准确率']
    metrics_values = [
    ...
    plt.suptitle('整体性能汇总', fontsize=18, fontweight='bold', y=0.98)
    plt.tight_layout()
...
def plot_keypoint_analysis(video_diagnostics):
    ...
    ax1.set_title('各视频平均可见关键点数', fontsize=14, fontweight='bold', pad=15)
    ax1.set_xlabel('视频', fontsize=12, fontweight='bold')
    ax1.set_ylabel('平均关键点数', fontsize=12, fontweight='bold')
    ax1.set_xticklabels(video_names, rotation=45, ha='right', fontsize=10)
    ax1.grid(True, axis='y', alpha=0.4)
    ax1.set_ylim([0, 18])
    ax1.axhline(y=12, color='red', linestyle='--', alpha=0.7, label='建议阈值 (12)')
    ax1.legend()
    ...
    bars = ax2.bar(video_names, confidences, color=plt.cm.viridis(np.linspace(0.3, 0.9, len(video_names))),
                   edgecolor='black', linewidth=1.5, alpha=0.9)
    ax2.set_title('各视频平均关键点置信度', fontsize=14, fontweight='bold', pad=15)
    ax2.set_xlabel('视频', fontsize=12, fontweight='bold')
    ax2.set_ylabel('平均置信度', fontsize=12, fontweight='bold')
    ax2.set_xticklabels(video_names, rotation=45, ha='right', fontsize=10)
    ax2.grid(True, axis='y', alpha=0.4)
    ax2.set_ylim([0, 1])
    ax2.axhline(y=0.3, color='orange', linestyle='--', alpha=0.7, label='阈值 (0.3)')
    ax2.legend()
    ...
    ax3.set_title('关键点与置信度相关性', fontsize=14, fontweight='bold', pad=15)
    ax3.set_xlabel('平均关键点数', fontsize=12, fontweight='bold')
    ax3.set_ylabel('平均置信度', fontsize=12, fontweight='bold')
    ...
    if all_visible_kpts:
        ...
        ax4.set_title('可见关键点分布 (所有帧)', fontsize=14, fontweight='bold', pad=15)
        ax4.set_xlabel('可见关键点数量', fontsize=12, fontweight='bold')
        ...
        ax4.axvline(x=mean_val, color='red', linestyle='-', alpha=0.8, linewidth=2, label=f'均值: {mean_val:.1f}')
        ax4.axvline(x=median_val, color='green', linestyle='--', alpha=0.8, linewidth=2,
                    label=f'中位数: {median_val:.1f}')
        ax4.legend()

    plt.suptitle('关键点检测分析', fontsize=18, fontweight='bold', y=0.98)
    plt.tight_layout()
